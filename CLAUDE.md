...
- **⚠️ O deploy automático do Coolify NÃO está funcionando (2026-08-21).** O webhook do GitHub está ativo e as entregas voltam **200 OK** — o Coolify recebe e não deploya. É configuração do lado dele (*Automatic Deployment* desligado ou branch errada). Consequência prática: **pushar não é deployar** neste projeto. Confirme sempre que o código subiu antes de dar algo como entregue (`curl` num endpoint que mudou, ou o hash do bundle em `/`).
...
- **Nós de SGP não têm bloco no PropsPanel** (`consultar_cliente`, `consultar_boleto`, `verificar_status`, `promessa_pagamento`, `listar_planos`) — **inconfiguráveis pela interface**. Consequência viva: o motor lê `cfg.pergunta` no `consultar_cliente` e não há como setá-la pela tela, então **o cliente nunca é perguntado pelo CPF** (o nó fica em silêncio esperando). Contorno: preencher no JSON do fluxo (funciona e sobrevive ao salvar).
- **Simulador diverge do motor** no `consultar_cliente`: [motorSimulador.js:88](apps/api/src/services/motorSimulador.js#L88) lê `cfg.mensagem` **com default embutido** `'Informe seu CPF:'`; o motor lê `cfg.pergunta`. A aba Simulação mostra uma pergunta que o motor real não enviaria — falso positivo de confiança justo no nó de entrada de dado.
