...
- ⚠️ **`salvar_dado` melhorou, mas ainda MENTE de vez em quando** (medido no container de produção, 2026-08-27, log correlacionado). Antes da correção ele nunca era chamado; hoje é chamado, mas só no primeiro turno com dados: nome, data de nascimento e plano foram salvos, e nos dois turnos seguintes ela respondeu *"Deixa eu anotar isso tudo"* e *"Anotei tudo"* sem chamar a tool — endereço, CEP, celular e e-mail ficaram só no histórico cru. O prompt já carrega a ordem nos dois casos, então **persuasão de prompt não fecha isto**; a garantia de verdade seria extrair os campos do turno fora do modelo. Sonda: `docker logs <container> | grep 'salvar_dado →'` — o que não aparecer ali não foi salvo, por mais que ela diga que anotou.
