...
apps/api/src/
  ...
  migrations/versions/   001..020 — modelo de dados (rode em ordem; NUNCA ALTER TABLE solto)
...
**Testes:** `cd apps/api && npm test` (runner nativo `node --test`, zero deps) — **388 testes puros**, rodam em qualquer máquina sem serviço nenhum. `motorFluxo.js` **não é importável em teste** (puxa `config/db.js` → Knex no topo e as deps não ficam instaladas localmente); por isso toda lógica testável vive em **módulos puros** ao lado dele — escreva o teste primeiro (TDD):
...
### Plano de Evolução V1.0 — 10 de 13 fases entregues
