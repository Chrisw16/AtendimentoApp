...
- **Pré-cadastro real (SGP):** `precadastrarCliente` grava em **modo lead** (`precadastro_ativar=0`; o SGP só exige `nome`+`logradouro`) — a equipe monta o contrato. Endpoint de planos correto é `/api/precadastro/plano/list` (o antigo `/api/ura/planos/` dava 404); `datanasc` normalizado p/ `AAAA-MM-DD`; `nas_id=53`. Detalhe em [brain/systems/maxxi/components/precadastro-real.md](brain/systems/maxxi/components/precadastro-real.md).
