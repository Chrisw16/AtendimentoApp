...
async function seed() {
  ...
  // ── CATÁLOGOS DAS FASES 5 a 9 ─────────────────────────────────
  // Moram em `dadosIniciais.js` e são semeados TAMBÉM pela migration 022,
  // porque o `seed` não roda no deploy — só as migrations. Aqui é só para
  // quem sobe o ambiente do zero pelo `npm run seed`.
  await semearCatalogos(db);

  // ── FLUXO PADRÃO ──────────────────────────────────────────────
  // ⚠️ O motor escolhe o fluxo com `where({ativo:true}).first()`, SEM ORDER BY.
  // Inserir um segundo fluxo ativo num ambiente que já atende faz o Postgres
  // poder entregar ESTE — que é legado e não roda no motor atual — para toda
  // conversa nova. Por isso a guarda é "existe algum fluxo ativo?", e não
  // "existe um chamado Atendimento Padrão?".
  const jaTemFluxoAtivo = await db('fluxos').where({ ativo: true }).first();
  const fluxoExiste = jaTemFluxoAtivo || await db('fluxos').where({ nome: 'Atendimento Padrão' }).first();
  if (!fluxoExiste) {
