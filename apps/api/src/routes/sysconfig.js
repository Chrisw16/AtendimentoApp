...
// GET /api/sysconfig — retorna todas as chaves permitidas
sysconfigRouter.get('/', asyncHandler(async (req, res) => {
  const db   = getDb();
  const rows = await db('sistema_kv')
    .whereIn('chave', CHAVES_PUBLICAS);
...
// PUT /api/sysconfig — salva múltiplas chaves de uma vez
sysconfigRouter.put('/', asyncHandler(async (req, res) => {
  const db = getDb();

  const updates = Object.entries(req.body)
    .filter(([k]) => CHAVES_PUBLICAS.includes(k));

  for (const [chave, valor] of updates) {
    await db('sistema_kv')
      .insert({ chave, valor: JSON.stringify(valor) })
      .onConflict('chave')
      .merge(['valor', 'atualizado']);
  }

  res.json({ ok: true, atualizado: updates.map(([k]) => k) });
}));

// GET /api/sysconfig/:chave — chave individual
sysconfigRouter.get('/:chave', asyncHandler(async (req, res) => {
  ...
  try {
    res.json({ valor: typeof row.valor === 'string' ? JSON.parse(row.valor) : row.valor });
  } catch {
