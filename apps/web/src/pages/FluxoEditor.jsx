...
function PropsPanel({ node, onChange, onDelete }) {
  ...
  // FASE 5: as filas de atendimento alimentam o select do `transferir_agente`.
  ...
  return (
    <div style={{width:270,background:'rgba(6,10,18,.98)',border:'1px solid rgba(255,255,255,.1)',borderRadius:12,display:'flex',flexDirection:'column',overflow:'hidden',boxShadow:'0 8px 32px rgba(0,0,0,.6)'}}>
      ...
      <div style={{padding:12,overflowY:'auto',flex:1,maxHeight:'calc(100vh - 160px)'}}>
        ...
        {node.data.tipo==='ia_roteador'&&(()=>{const rotas=Array.isArray(cfg.rotas)?cfg.rotas:[];const setR=r=>set('rotas',r);return(<><Fld label="Mensagem inicial"><textarea value={cfg.mensagem||''} onChange={e=>set('mensagem',e.target.value)} rows={2} placeholder="Posso te ajudar com mais alguma coisa? 😊" style={TA}/></Fld><Fld label="Rotas (intenções)" hint="Cada rota = uma porta de saída">{rotas.map((r,i)=><div key={i} style={{marginBottom:8,padding:'8px',background:'rgba(232,121,249,.05)',borderRadius:7,border:'1px solid rgba(232,121,249,.15)'}}><div style={{display:'flex',gap:5,marginBottom:5}}><div style={{flex:1}}><div style={{fontSize:9.5,color:'rgba(255,255,255,.3)',marginBottom:2}}>ID da porta</div><input value={r.id||''} onChange={e=>{const n=[...rotas];n[i]={...n[i],id:e.target.value.toLowerCase().replace(/\s+/g,'_').replace(/[^a-z0-9_]/g,'')};setR(n);}} placeholder="boleto" style={{...IS,fontSize:11,fontFamily:'monospace'}}/></div><div style={{flex:2}}><div style={{fontSize:9.5,color:'rgba(255,255,255,.3)',marginBottom:2}}>Label</div><input value={r.label||''} onChange={e=>{const n=[...rotas];n[i]={...n[i],label:e.target.value};setR(n);}} placeholder="2ª via de boleto" style={{...IS,fontSize:11}}/></div><button onClick={()=>{const n=[...rotas];n.splice(i,1);setR(n);}} style={{background:'none',border:'none',color:'#ff4757',cursor:'pointer',fontSize:15,padding:'0 4px',flexShrink:0,alignSelf:'flex-end',marginBottom:2}}>×</button></div><div style={{fontSize:9.5,color:'rgba(255,255,255,.3)',marginBottom:2}}>Descrição para a IA</div><input value={r.descricao||''} onChange={e=>{const n=[...rotas];n[i]={...n[i],descricao:e.target.value};setR(n);}} placeholder="quando o cliente quer ver ou pagar boleto" style={{...IS,fontSize:10.5}}/></div>)}<button onClick={()=>setR([...rotas,{id:'',label:'',descricao:''}])} style={{width:'100%',padding:'6px 0',background:'rgba(232,121,249,.06)',border:'1px dashed rgba(232,121,249,.3)',borderRadius:6,color:'#e879f9',fontSize:11,cursor:'pointer'}}>+ Adicionar rota</button></Fld><Fld label="Prompt de recepção" hint="Slug da aba Prompts IA. Vazio usa 'roteador'.">
          <input value={cfg.contexto||''} onChange={e=>set('contexto',e.target.value)} placeholder="roteador" style={IS}/></Fld><Fld label="Máx. turnos" hint="Quantas trocas a recepção pode ter antes de desistir e sair pela porta 'não entendeu'.">
          <input type="number" value={cfg.max_turnos??''} onChange={e=>e.target.value===''?set('max_turnos',undefined):set('max_turnos',parseInt(e.target.value)||0)} placeholder="6" style={{...IS,width:80}}/></Fld><div style={{fontSize:10,color:'rgba(255,255,255,.35)',lineHeight:1.5,padding:'6px 0'}}>Este nó conversa: ele cumprimenta, entende o pedido, identifica o cliente pelo CPF e só então encaminha. Memória, base de conhecimento e identificação estão sempre ligadas — não há o que configurar.</div></>);})()}

        {node.data.tipo==='transferir_agente'&&<><Fld label="Motivo" hint="Só documenta o nó — não vai para lugar nenhum"><textarea value={cfg.motivo||''} onChange={e=>set('motivo',e.target.value)} rows={2} placeholder="Cliente solicitou atendimento humano" style={TA}/></Fld><Fld label="Fila de destino" hint="Vazio = sem fila (todos os agentes veem). A fila define o SLA e o horário."><select value={cfg.fila||''} onChange={e=>set('fila',e.target.value)} style={{...IS,cursor:'pointer'}}><option value="">— sem fila —</option>{filas.map(f=><option key={f.id} value={f.slug}>{f.nome}{f.ativa?'':' (inativa)'}</option>)}</select>{cfg.fila&&!filas.some(f=>f.slug===cfg.fila)&&<div style={{fontSize:9.5,color:'#ff6b35',marginTop:3}}>A fila "{cfg.fila}" não existe mais — a transferência vai cair sem fila.</div>}</Fld><Fld label="Mensagem fora do horário"><textarea value={cfg.msg_fora||''} onChange={e=>set('msg_fora',e.target.value)} rows={2} placeholder="Atendemos seg-sex 8h-18h." style={TA}/></Fld></>}
